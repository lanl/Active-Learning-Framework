...
class ensemblemolecule(object):
    ...
    def compute_mean_props(self, disable_ani=False):
        ...
        self.Fstddev = np.mean(np.std(self.F, axis=0))

        # Return
        return np.mean(self.E, axis=0), np.mean(self.F, axis=0), np.std(self.E, axis=0) / np.sqrt(float(self.Na)), self.Fstddev

    def get_Fstddev(self):
        return self.Fstddev, np.std(self.F, axis=0).max()
    ...
    def compute_sigma_bias_potential(self, X, S, Efunc, Ffunc, enable_energy_bias=True, enable_force_bias=True, bias_atoms = False, epsilon=0.001, disable_ani=False):
        if self.Nn < 2:
            print('Error: compute_sigma_bias_potential requires more than 1 network in the ensemble.')
            raise ValueError
        ...
        if enable_energy_bias:
            ...
            E_v2 = 0.0
            E_dx = 0.0*np.sum(F,axis=0)

        # --------- FORCE BIAS ----------
        # dF and C
        if enable_force_bias:
            dF = F - np.mean(F, axis=0)[np.newaxis, :, :]
            ...
            F_dx = 0.0*np.sum(F,axis=0)
            F_v2 = 0.0

        E_bias = Efunc(F_v2, E_v2)
        F_bias = Ffunc(F_v2, F_dx, E_v2, E_dx)
        if bias_atoms:
            bias_mask = np.array([[ 1.0 if x in bias_atoms else 0.0 for x in range(F_bias.shape[0]) ]])
            #print(bias_mask)
            #print(F_bias)
            #print(F_bias*bias_mask.T)
            F_bias = F_bias*bias_mask.T
            #print(F_bias)
...
class batchedensemblemolecule(object):
    def __init__(self, cnstfile, saefile, nnfprefix, Nnet, gpuid=0, sinet=False, forcesigma=False, enablepairwise=False, net_start_id=0):
        ...
        self.cnstfile = cnstfile
        self.saefile  = saefile
        self.nnfprefix = nnfprefix
        self.gpuid = gpuid
        self.sinet = sinet
    ...
    def set_pbc(self, pbc0, pbc1, pbc2):
        self.pbc = np.array([pbc0,pbc1,pbc2])
        self.ncl.setPBC(pbc0, pbc1, pbc2)

    def set_cell(self, cell, pbc_inv):
        self.cell = cell
        self.pbc_inv = pbc_inv
    ...
    def compute_stress_virial(self):
        self.Wi = self.ncl.get_atomic_virials().copy()
        W = np.sum(self.Wi,axis=0)
        W_v = np.array([W[0,0],W[1,1],W[2,2],W[1,2],W[0,2],W[0,1],]) 

        return W_v, 0 # nEGATIVE NEEDS TO BE FIXED INSIDE NEUROCHEM rJ-rI
    ...
    def compute_mean_props(self, disable_ani=False):
        ...
        # Return
        return self.E[0], self.F, self.ncl.esigma()[0]/np.sqrt(self.Na), None

    def get_Fstddev(self):
        if self.naive_ncl == None:
            net_start_id = 0
            self.naive_ncl = [pync.molecule(self.cnstfile, self.saefile, self.nnfprefix + "train" + str(i+net_start_id) + '/networks/', 1, self.gpuid, self.sinet) for i in range(self.Nn)]

        for nc in self.naive_ncl:
            nc.setMolecule(coords=np.array(self.X,dtype=np.float64), types=list(self.S))

        for nc in self.naive_ncl:
            nc.setPBC(self.pbc[0], self.pbc[1], self.pbc[2])

        for nc in self.naive_ncl:
            nc.setCell(self.cell, self.pbc_inv)

        self.Ene = np.zeros((self.Nn), dtype=np.float64)
        self.Fne = np.zeros((self.Nn, self.Na, 3), dtype=np.float64)
        for i, nc in enumerate(self.naive_ncl):
            self.Ene[i] = nc.energy().copy()
            self.Fne[i] = nc.force().copy()

        # dF and C
        dF = self.Fne - np.mean(self.Fne, axis=0)[np.newaxis, :, :]

        # Compute var sqr
        E_v2 = 0.5*np.sum(np.power(self.Ene-np.mean(self.Ene),2.0))
        F_v2 = np.sum(np.sum(np.power(dF, 2), axis=0) / (self.Nn * (self.Nn - 1)))
        ...
        # Return
        return np.mean(np.std(self.Fne, axis=0)), np.std(self.Fne, axis=0).max()
...
class ANIENS(Calculator):
    ...
    def get_Fstddev(self):
        Fstddev, Fstddev_max = self.nc.get_Fstddev()
        return self.energy_conversion * Fstddev, self.energy_conversion * Fstddev_max
    ...
    def set_sigmabias(self,bias_Efunc, bias_Ffunc, enable_energy_bias=True, enable_force_bias=True, bias_atoms=False, epsilon=0.001, disable_ani=False):
        self.bias_Efunc = bias_Efunc
        self.bias_Ffunc = bias_Ffunc
        self.enable_energy_bias = enable_energy_bias
        self.enable_force_bias = enable_force_bias
        self.bias_atoms = bias_atoms
    ...
    def calculate(self, atoms=None, properties=['energy'],
        ...
        if not self.add_bias:
            ...
            energy, force, stddev, Fstddev = self.nc.compute_sigma_bias_potential(self.atoms.get_positions().astype(np.float64),
                                                                                  ...
                                                                                  self.enable_energy_bias, self.enable_force_bias, self.bias_atoms,
        ...
        self.results['free_energy'] = self.energy_conversion * energy
        self.Emodel = energy
        ...
        self.results['stress'] = -self.energy_conversion * stress_ani/V # CORRECT
        #self.results['stress'] = -stress_ani
